    ...
    <div x-data="{
                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="mb-4 border-b border-gray-200">
                        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                            <button @click.prevent="tab = 'details'"
                            ...
                            <button @click.prevent="tab = 'variants'"
                            ...
                            <button @click.prevent="tab = 'images'"
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <x-text-input id="name" name="name" type="text"
                                                class="mt-1 block w-full" :value="old('name', $product->name)" required autofocus />
                                        </div>
                                        ...
                                        <div class="block mt-4">
                                            <label for="status" class="inline-flex items-center">
                                                ...
                                                <span
                                                    class="ml-2 text-sm text-gray-600">{{ __('Công khai sản phẩm (Hiển thị cho khách hàng)') }}</span>
                                            </label>
                                        </div>
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6 text-gray-900">
                                ...
                                <div class="mt-4 mb-4 flex justify-end">
                                    <x-primary-button @click.prevent="openVariantModal(true)">
                                ...
                                <div class="overflow-x-auto shadow-md sm:rounded-lg">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @forelse ($product->variants as $variant)
                                                <tr>
                                                    ...
                                                    <td
                                                        ...
                                                        <form class="inline"
                                                            action="{{ route('admin.variants.destroy', ['product' => $product, 'variant' => $variant]) }}"
                                                            method="POST"
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">Chưa
                                                        có biến thể nào.</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6 text-gray-900">
                                ...
                                <p class="mt-2 text-sm text-gray-600 mb-4">
                                    Upload nhiều ảnh cho sản phẩm. Các ảnh này sẽ hiển thị trong thư viện ảnh (gallery)
                                    của trang chi tiết sản phẩm.
                                </p>

                                <form action="{{ route('admin.images.store', $product) }}" method="POST"
                                ...
                                <div class="mt-8">
                                    ...
                                    @if ($product->images->isEmpty())
                                        <p class="mt-2 text-sm text-gray-500">Sản phẩm này chưa có ảnh nào trong thư
                                            viện.</p>
                                    @else
                                        <div class="mt-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                            @foreach ($product->images as $image)
                                                <div classs="relative group border rounded-lg overflow-hidden">
                                                    ...
                                                    <div class="p-2">
                                                        <form
                                                            action="{{ route('admin.images.destroy', ['product' => $product, 'image' => $image]) }}"
                                                            method="POST"
                                                            onsubmit="return confirm('Bạn có chắc chắn muốn xóa ảnh này?');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type-="submit"
                                                                class="text-red-600 hover:text-red-900 text-sm w-full text-center">
                                                                Xóa ảnh
                                                            </button>
                                                        </form>
                                                    </div>
        ...
        <x-modal name="variant-modal" max-width="lg">
            <template x-if="editVariant">
                ...
                <form
                    :action="isCreatingVariant ? '{{ route('admin.variants.store', $product) }}' :
                        `/admin/variants/${editVariant.id}`"
                    ...
                    <div class="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="var_color" name="color" type="text" class="mt-1 block w-full"
                                x-model="editVariant.color" placeholder="Vd: Đen, Trắng/Đỏ" />
                        </div>

                        <div>
                            <x-input-label for="var_size" :value="__('Size (tùy chọn)')" />
                            <x-text-input id="var_size" name="size" type="text" class="mt-1 block w-full"
                                x-model="editVariant.size" placeholder="Vd: 41, 42.5, L" />
                        </div>

                        <div>
                            <x-input-label for="var_price" :value="__('Giá (bắt buộc)')" />
                            <x-text-input id="var_price" name="price" type="number" step="1000"
                                min="0" class="mt-1 block w-full" x-model="editVariant.price" required />
                        </div>

                        <div>
                            <x-input-label for="var_stock" :value="__('Số lượng Tồn kho (bắt buộc)')" />
                            <x-text-input id="var_stock" name="stock_quantity" type="number" min="0"
                                class="mt-1 block w-full" x-model="editVariant.stock_quantity" required />
                        </div>

                        <div class="sm:col-span-2">
                            <x-input-label for="var_image" :value="__('Ảnh Đại Diện (cho màu này)')" />
                            <template x-if="!isCreatingVariant && editVariant.image_url">
                                <img :src="editVariant.image_url" alt="Ảnh hiện tại"
